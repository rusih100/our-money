...
src/
  ...
  lib/
    ...
    storage.ts          персистентность в app-data (categories.json, initiators.json, progress/<key>.json).
  ...
  components/
    ...
    SummaryPanel.tsx    живая аналитика (need/want, повторяемость, по категориям/инициаторам, исключённые).
    ManagedListPanel.tsx переиспользуемая модалка управления упорядоченным списком.
    CategoryPanel.tsx   обёртка ManagedListPanel для категорий (хоткеи 1–9).
    InitiatorPanel.tsx  обёртка ManagedListPanel для инициаторов (хоткеи ⇧1–9).
...
## Аннотации (6 новых колонок, добавляются на экспорте; оригиналы не меняются)

| Колонка | Тип | Смысл |
|---|---|---|
| `true_category` | string | категория из управляемого списка |
| `initiator` | string | инициатор траты (имя) из управляемого списка |
| `is_recurring` | bool | повторяющийся vs разовый |
| `need_want` | `need` \| `want` | назначение траты |
| `note` | string | свободная заметка |
| `annotated` | bool | строка была размечена (прогресс/резюме) |

Плюс флаг `excluded` (bool, «не учитывать») — в экспорт **не** выводится отдельной
колонкой: строки с `excluded` целиком пропускаются `serializeCsv`. Исключённые
строки считаются размеченными (двигают прогресс).

## Горячие клавиши

`j`/`↓` дальше · `k`/`↑` назад · `1`–`9` категория по позиции · `⇧1`–`⇧9`
инициатор по позиции · `r` recurring · `n`/`w` need/want · `x` не учитывать
(+переход дальше) · `c` фокус на заметку (`Esc` назад) · `Enter`
...
2. **Оригинальные колонки неизменны.** Разметка только добавляет новые колонки;
   ...
   форматирование round-trip-ятся). Строки с флагом `excluded` («не учитывать»)
   целиком пропускаются на экспорте.
...
- `initiators.json` — глобальный список инициаторов (аналогично категориям).
