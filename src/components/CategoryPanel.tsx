...
export function CategoryPanel(): React.ReactElement | null {
  ...
  return (
    <div
      ...
      onMouseDown={() => setShowCategories(false)}
    >
      <div
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-5 py-3">
          <h2 className="text-sm font-semibold text-fg">Категории</h2>
          <button
            ...
            categories.map((cat, i) => (
              ...
              >
                ...
                <input
                  ...
                  onBlur={(e) => {
                    setEditing("none");
                    if (e.target.value.trim() && e.target.value !== cat) {
        ...
        <div className="border-t border-border px-5 py-2 text-xs text-fg-faint">
          Порядок задаёт хоткеи 1–9. Изменения сохраняются автоматически.
        </div>
      </div>
    </div>
  );
